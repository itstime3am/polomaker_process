...
class Mdl_screen_process extends MY_Model {
	function __construct() {
		...
		$this->_TABLE_NAME = 'pm_t_manu_production';
	...
	function search($arrObj = array()) {
		...
		$_sql = <<<EOT
		select o.quotation_detail_rowid ,d.type_id, d.order_rowid, CONCAT(o.category, o.type, ' [ ', o.job_number, ' ] ') AS disp_order
		, d.position, d.detail, d.size, d.job_hist, s.screen_type, s.name AS disp_type, ss."name"  as disp_status,
		 tmp.rowid  as prod_id, tmp.prods_rowid  as prods_rowid
		, ARRAY_TO_JSON(ARRAY(
...
*/
	...
	function change_status_by_id($rowid, $ps_rowid, $status_remark = FALSE, $order_rowid, $type_id) {
		...
		$this->db->set('prods_rowid', $_ps_rowid);
