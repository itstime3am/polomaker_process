	$(function() {
		...
		doPopulateTable = function (arrData, blnChangeSearchCriteria) {
			...
			//$('#divDisplayQueryResult').html('<table id="tblSearchResult" class="cls-tbl-list"><thead><tr><th rowspan="2">สถานะ</th><th rowspan="2">เลขที่ใบสั่งตัด</th><th rowspan="2">เซลส์</th><th rowspan="2">ชื่อลูกค้า</th><th rowspan="2">บริษัท</th><th rowspan="2">กลุ่มสินค้า</th><th rowspan="2">ประเภทสินค้า</th><th rowspan="2">กำหนดส่ง</th><th rowspan="2">วันที่ส่งลูกค้า</th><th rowspan="2" >วันที่ส่งจริง</th><th rowspan="2">จำนวนตัว</th><th rowspan="2">ราคารวม</th><th rowspan="2">VAT</th><th rowspan="2">รวมทั้งสิ้น</th><th colspan="3" class="ui-state-default">มัดจำ</th><th colspan="5" class="ui-state-default">งวดสุดท้าย</th></tr><tr><th>จำนวนเงิน</th><th>ช่องทางการชำระเงิน</th><th>วันที่ชำระเงิน</th><th>จำนวนเงิน</th><th>ช่องทางการชำระเงิน</th><th>วันที่ชำระเงิน</th><th>WHT จำนวน</th><th>หมายเหตุ</th></tr></thead><tbody></tbody></table>');
	...
	if (_ALLOW_EDIT === false) {
		...
		_doSetEditableColumns = function() {
			$('#tblSearchResult tbody tr td.order_status').editable(
			...
			$('#tblSearchResult tbody tr td.status_deliver_date').editable(
				function (value, settings) { 
					return (_doPrepareChangedData('status_deliver_date', value, this));
				}, { type: 'datepicker', onblur: 'submit', tooltip: 'Click to edit...' }
			);

			$('#tblSearchResult tbody tr td.close_payment_amount').editable(
				function (value, settings) { 
					if (_doPrepareChangedData('close_payment_amount', value, this)) {
						if (isNaN(value)) {
							return value;
						} else {
			...
			$('#tblSearchResult tbody tr td.close_payment_route_id').editable(
				function (value, settings) { 
					if (_doPrepareChangedData('close_payment_route_id', value, this) !== false) {
						return (settings.data[value]);
					}
					return false;
				}, { type: 'select', data: _objClosePaymentRouteOptions, onblur: 'submit', tooltip: 'Click to edit...' }
			);
			
			$('#tblSearchResult tbody tr td.close_payment_date').editable(
				function (value, settings) { 
					return (_doPrepareChangedData('close_payment_date', value, this));
				}, { type: 'datepicker', onblur: 'submit', tooltip: 'Click to edit...' }
			);
			
			$('#tblSearchResult tbody tr td.close_payment_wht').editable(
				function (value, settings) { 
					return (_doPrepareChangedData('close_payment_wht', value, this));
				}, { type: 'text', onblur: 'submit', tooltip: 'Click to edit...' }
			);

			$('#tblSearchResult tbody tr td.deliver_remark').editable(
				function (value, settings) { 
					return (_doPrepareChangedData('deliver_remark', value, this));
				}, { type: 'text', onblur: 'submit', tooltip: 'Click to edit...' }
			);
		};
	}
	...
	function _doPrepareChangedData(data_field, value, objSource) {
		...
		if ((_tr) && (_datatable)) {
			...
			if (('order_rowid' in _aData) && ('order_type_id' in _aData) && (data_field) && (value)) {
				...
				_objChangedData[_str_index][data_field] = value;
