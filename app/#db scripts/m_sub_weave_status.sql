...
-- DROP TABLE IF EXISTS m_sub_weave_status;

CREATE TABLE IF NOT EXISTS m_sub_weave_status (
...
CREATE OR REPLACE RULE rule_d_m_sub_weave_status AS
  ON DELETE TO m_sub_weave_status
  DO INSTEAD
    UPDATE m_sub_weave_status 
	SET is_cancel = 1
	, update_by = -1
...
GRANT ALL ON TABLE m_sub_weave_status TO polomaker_db;

------------------------------------------------------------------------

-- TRUNCATE TABLE m_sub_weave_status;

INSERT INTO m_sub_weave_status(rowid, code, name, description, create_by, create_date) VALUES
